(function () {
    ...
    window.addToWishlist = function (productId) {
        fetch(window.addToWishlistApiUrl, {
            ...
            .then(data => {
                ...
                document.querySelectorAll(`button.wishlist-toggle-${productId}`).forEach(function (btn) {
                    btn.setAttribute("onclick", `removeFromWishlist(${productId})`);
                    const icon = btn.querySelector("i");
                    if (icon) {
                        icon.className = "ci-heart-filled fs-base animate-target";
                    }
                });
                ...
                fetch(window.wishlistPartialUrl, {
                    ...
                    .then(htmlSnippet => {
                        document.querySelectorAll('.wishlistContainer').forEach(function (el) {
    ...
    window.removeFromWishlist = function (productId) {
        fetch(window.removeFromWishlistApiUrl, {
            ...
            .then(data => {
                ...
                document.querySelectorAll(`button.wishlist-toggle-${productId}`).forEach(function (btn) {
                    btn.setAttribute("onclick", `addToWishlist(${productId})`);
                    const icon = btn.querySelector("i");
                    if (icon) {
    ...
    window.addToCart = function (productId, count) {
        fetch(window.addToCartApiUrl, {
            ...
            .then(htmlSnippet => {
                if (htmlSnippet) {
                    document.querySelectorAll('.cartContainer').forEach(function (el) {
                        el.innerHTML = htmlSnippet;
                    });
                }
            })
    ...
    window.loadOrderDetails = function loadOrderDetails(orderId) {
        ...
        fetch(url, {
            ...
            .then(html => {
                ...
                const bsOffcanvas = bootstrap.Offcanvas.getOrCreateInstance(container);
                bsOffcanvas.show();
            })
            .catch(err => {
    ...
    $('#loginModel').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            ...
            error: function () {
                $('#authenticateContent').html('<p>An error occurred while loading the login form. Please try again later.</p>');
            }
        });
    });
    ...
    $('#addressModel').on('show.bs.modal', function (event) {
        ...
        $.ajax({
            ...
            error: function () {
                $('#addressModelContent').html('<p>An error occurred while loading the login form. Please try again later.</p>');
            }
        });
    });
    ...
    $(document).on('submit', '#loginByCodeForm', function (e) {
        ...
        $.ajax({
            ...
            error: function () {
                $('#authenticateContent').html('<p>An error occurred while processing your request. Please try again later.</p>');
            }
        });
    });
    ...
    $(document).on('click', '#loginByCodeLink', function (e) {
        ...
        $.ajax({
            ...
            error: function () {
                $('#authenticateContent').html('<p>An error occurred while loading form. Please try again later.</p>');
            }
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        loadMoreBtn.addEventListener('click', function () {
            ...
            fetch(`${window.infiniteScrollUrl}?page=${nextPage}&category=${encodeURIComponent(categoryFilter)}`, {
                headers: {
        ...
        fetch(window.deleteAddress, {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    document.getElementById('address').innerHTML = data.updated_html;
                } else {
                    Swal.fire({
                        title: data.title,
                        text: data.message,
